  ...
  cy.filterForTransactionsService(numContract).then(value => {
    ...
    cy.request({
      ...
      if (response.body.total > 1) {
        var a = dayjs(JSON.stringify(response.body.data[1].FechaFin), 'DD/MM/YYYY').add(1, 'day').format('DD/MM/YYYY');
        var b = dayjs(JSON.stringify(response.body.data[0].FechaInicio), 'DD/MM/YYYY').format('DD/MM/YYYY');
        expect(a).to.equal(b);
        return cy.wrap(response.body);
      }
    })
